...
export async function handleGenerate(prompt: string): Promise<void> {
    ...
    try {
        ...
        if (isMultiFileResponse(fullText)) {
            const { files: parsedFiles, deletions, summary } = parseMultiFileResponse(fullText);
        ...
        ui.printError(`Generation failed: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export async function handleChat(initialPrompt?: string): Promise<void> {
    ...
    ui.printHeader();
    ui.printInfo(`Starting interactive chat session with ${ui.colors.bold.cyan(config.model)}.`);
    ui.printInfo('Type your prompt to generate/edit files. Type "exit" or "quit" to end the session.\n');
    ...
    async function runTurn(userPrompt: string) {
        ...
        try {
            ...
            spinner.stop();
            ui.printError(`Error during turn: ${error instanceof Error ? error.message : String(error)}`);
            console.log();
        }
