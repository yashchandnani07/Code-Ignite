...
export async function handleDoctor(): Promise<void> {
    ...
    const configPath = path.join(process.env.HOME || process.env.USERPROFILE || '', '.codeignite', 'config.json');
    if (fs.existsSync(configPath)) {
        ui.printSuccess(`Found config file at: ${configPath}`);
    } else {
        ui.printWarning('No configuration file found. Run "codeignite login" to initialize.');
        issuesCount++;
    }
    console.log();
    ...
    if (!config.apiKey && config.provider !== 'OpenRouter') {
        ui.printError('API Key is missing. The CLI agent will not be able to generate code.');
        issuesCount++;
    } else {
        ui.printSuccess('API Key is set in configuration.');
        
        // Validate Key connection
        const spinner = ui.createSpinner('Testing connection to AI Provider API...');
        spinner.start();
        try {
            const res = await fetchLatestModels(config.provider, config.apiKey, config.baseUrl);
            spinner.stop();
            if (res.models && res.models.length > 0) {
                ui.printSuccess('Successfully connected to AI Provider API! Key is valid.');
            } else {
                throw new Error('Received empty models listing.');
            }
        } catch (error) {
            spinner.stop();
            ui.printError(`API connection test failed: ${error instanceof Error ? error.message : String(error)}`);
            ui.printWarning('Please verify your API key, internet connection, or base URL override.');
            issuesCount++;
        }
    }
    console.log();
    ...
    if (indexExists) {
        ui.printSuccess('Found index.html (Valid Code Ignite workspace entry point).');
    } else {
        ui.printInfo('No index.html found. Ready to generate a new project in this directory.');
    }
    ...
    if (issuesCount === 0) {
        ...
        ui.printWarning(`Doctor Diagnostic: Found ${issuesCount} potential issue(s). Run "codeignite login" to resolve config issues.`);
    }
    console.log();
}
