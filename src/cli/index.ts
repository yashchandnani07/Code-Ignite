...
program
    .command('chat [prompt]')
    .description('Start an interactive chat session or send a single prompt with conversation context')
    .action(async (prompt) => {
...
program
    ...
    .description('Generate or modify workspace files directly using a single prompt')
    .action(async (prompt) => {
...
program
    .command('build')
    .description('Run compile / build scripts for the current workspace if configured')
    .action(async () => {
        await handleBuild();
    });
...
program
    .command('preview')
    .description('Run a local static web server to preview index.html in the browser')
...
program
    ...
    .option('--provider <provider>', 'Deployment provider: "netlify" or "gist"', 'netlify')
...
program
    .command('login')
    .description('Configure API credentials and preferred providers interactively')
    .action(async () => {
        await handleLogin();
    });
...
program
    .command('doctor')
    .description('Diagnose and verify local credentials, configurations, and project setups')
    .action(async () => {
        await handleDoctor();
    });
