 ...
 * All resolved paths are validated to reside strictly within `dir` to
 * prevent directory-traversal attacks via AI-generated file paths.
 */
export function writeWorkspace(
  ...
  const resolvedWorkspaceDir = path.resolve(dir);
  const workspaceBoundary = resolvedWorkspaceDir + path.sep;

  // 1. Handle additions and modifications
  Object.entries(files).forEach(([relPath, content]) => {
    const absPath = path.resolve(resolvedWorkspaceDir, relPath);

    // Security: block directory traversal outside the workspace
    if (
      absPath !== resolvedWorkspaceDir &&
      !absPath.startsWith(workspaceBoundary)
    ) {
      ui.printWarning(`Blocked attempt to write outside workspace: ${relPath}`);
      return;
    }
  ...
  deletions.forEach((relPath) => {
    ...
    if (
      ...
      ui.printWarning(
        `Blocked attempt to delete outside workspace: ${relPath}`,
      );
      return;
    }
    ...
    if (fs.existsSync(absPath)) {
      try {
        ...
        ui.printWarning(
          `Failed to delete file: ${relPath}. ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  });
